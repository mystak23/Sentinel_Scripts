    ...
    Try {
        ...
        if($null -eq $AzureLogicApps){
            ...
            foreach($LogicApp in $AzureLogicApps){                               
                ...
                FixJsonIndentation -jsonOutput $armTemplateOutput | Set-Content "$($FolderName)\$($PlaybookResourceName).json"
                Write-Log -Message "ARM Template created successfully at $($FolderName)\$($PlaybookResourceName).json" -LogFileName $LogFileName -Severity Information                
            }
