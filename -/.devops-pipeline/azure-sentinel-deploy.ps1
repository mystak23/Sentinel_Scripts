Param(
    ...
    [string] [Parameter(Mandatory = $true)] $ContentTypes,
    [string] [Parameter(Mandatory = $true)] $systemAccessToken,
    [string] [Parameter(Mandatory = $true)] $branchName,
    [string] [Parameter(Mandatory = $true)] $repositoryName,
    [string] [Parameter(Mandatory = $true)] $collectionUri,
    [string] [Parameter(Mandatory = $true)] $projectId,
    [string] [Parameter(Mandatory = $true)] $sourceControlId,
    [string] [Parameter(Mandatory = $true)] $rootDirectory,
    [string] [Parameter(Mandatory = $true)] $smartDeployment,
...
$sentinelResourcePatterns = @{
    ...
    "DataCollectionRule" = "/subscriptions/$guidPattern/resourceGroups/$namePattern/providers/Microsoft.Insights/dataCollectionRules/$namePattern"
}
...
$DefinedParameters = "ParameterFile.json"
@"
{
...
"@ | Out-File -FilePath $DefinedParameters
...
$csvPath = "$rootDirectory\.devops-pipeline\tracking_table_$sourceControlId.csv"
...
function LoadDeploymentConfig() {
    ...
    $global:parameterFileMapping = @{
        '6-Playbooks/pb.custom-MFA_Request_Rejected_Historyjson' = $DefinedParameters 
